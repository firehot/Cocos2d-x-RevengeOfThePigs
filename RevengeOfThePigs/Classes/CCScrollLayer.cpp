...
namespace cocos2d 
{
	...
	bool CCScrollLayer::initWithLayers(CCArray* layers, int widthOffset)
	{
		...
		m_fMinimumTouchLengthToSlide = CCDirector::sharedDirector()->getWinSize().width/60.0f;;
		...
		m_tPagesIndicatorPosition = ccp(0.5f * m_tContentSize.width, ceilf(m_tContentSize.height / 16.0f));
	...
	void CCScrollLayer::visit()
	{
        ...
        CCDirector::sharedDirector()->getOpenGLView()->setScissorInPoints(winSize.width/24, 0, winSize.width*11/12, winSize.height);
        CCLayer::visit();	//< Will draw after glPopScene. 
        glDisable(GL_SCISSOR_TEST); 

		if (m_bShowPagesIndicator && false)
		{
	...
	{	
        ...
        float durMoveTime = fabsf(this->getPosition().x-positionForPageWithNumber(pageNumber).x)/800;
        if ( durMoveTime == 0 )
            return;
        
		CCAction* changePage = 
			CCSequence::actionOneTwo(
					CCMoveTo::actionWithDuration(durMoveTime, positionForPageWithNumber(pageNumber)),
	...
	bool CCScrollLayer::ccTouchBegan(CCTouch* pTouch, CCEvent* pEvent)
	{
        ...
        m_fStartPosX = this->getPosition().x;
	...
	void CCScrollLayer::ccTouchMoved(CCTouch* pTouch, CCEvent* pEvent)
	{
		...
		if (m_iState == kCCScrollLayerStateSliding)
		{
			...
			if ((page == 0 && offset > 0) || (page == m_pLayers->count() - 1 && offset < 0))
				...
				offset = 0;
			setPosition(ccp(m_fStartPosX + desiredX - offset, 0));
		}
	}
